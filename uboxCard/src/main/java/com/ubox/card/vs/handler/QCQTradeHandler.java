...
public class QCQTradeHandler implements Handler {
	...
	public Map<String, Object> readLoacalData(Map<String, Object> requestData) {
		...
		List<Object> tradeList = QCQDao.getInstance().queryLatestMaxN(30);
		if (tradeList != null && tradeList.size() != 0) {
			((Map) contentsData).put(VsConst.MAP_KEY_KCQ_TRADE, tradeList);
		} else {
			((Map) contentsData).put(VsConst.MAP_KEY_KCQ_TRADE, new ArrayList<Object>());
		}
	...
	public void deleteSynSuccessData(Map<String, Object> responseData) {
		...
		if (contentsData instanceof Map) {
			Object mztTradeIdList = ((Map)contentsData).get(VsConst.MAP_KEY_KCQ_TRADE_SUCCESS);
			if (mztTradeIdList instanceof List && ((List) mztTradeIdList).size() > 0) {
				...
				QCQDao.getInstance().deleteMany(ids);
				QCQDao.getInstance().init(); // BaseDao.records和db文件内容同步
			}
		} else {
