...
public final class WHTTradeHandler implements Handler{
	...
	public Map<String, Object> readLoacalData(Map<String, Object> requestData) {
		...
		List<Object> tradeList = WHTTradeDao.getInstance().queryLatestMaxN(30);
		if (tradeList != null && tradeList.size() != 0) {
			((Map) contentsData).put(VsConst.MAP_KEY_WHTDT_TRADE, tradeList);
		} else {
			((Map) contentsData).put(VsConst.MAP_KEY_WHTDT_TRADE, new ArrayList<Object>());
		}
	...
	public void deleteSynSuccessData(Map<String, Object> responseData) {
		...
		if (contentsData instanceof Map) {
			Object mztTradeIdList = ((Map)contentsData).get(VsConst.MAP_KEY_WHTDT_TRADE_SUCCESS);
			if (mztTradeIdList instanceof List && ((List) mztTradeIdList).size() > 0) {
				...
				WHTTradeDao.getInstance().deleteMany(ids);
				WHTTradeDao.getInstance().init(); // BaseDao.records和db文件内容同步
			}
		} else {
