...
public class UnionpayHandler implements Handler {
    ...
    public Map<String, Object> readLoacalData(Map<String, Object> requestData) {
		...
		List<Object> unionpayList = UnionpayTradeDao.getInstance().queryLatestMaxN(maxN);
		if (unionpayList != null && unionpayList.size() != 0) {
			((Map) contentsData).put(VsConst.MAP_KEY_UNIONPAY_TRADE, unionpayList);
		} else {
			// db中为空,传空值给服务器
			((Map) contentsData).put(VsConst.MAP_KEY_UNIONPAY_TRADE, new ArrayList<Object>());
		}
    ...
    public void deleteSynSuccessData(Map<String, Object> responseData) {
        ...
        if (contentsData instanceof Map) {
            Object unionpaySeqList = ((Map) contentsData).get(VsConst.MAP_KEY_UNIONPAY_TRADE_SUCCESS);
            if (unionpaySeqList instanceof List && ((List) unionpaySeqList).size() > 0) {
                ...
                UnionpayTradeDao.getInstance().deleteMany(ids);
                UnionpayTradeDao.getInstance().init();
                Logger.info("UnionpayTradeDao init OK.");
            }
        } else {
