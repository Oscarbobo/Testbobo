...
public class UBOXSER3 {
	 ...
	 * @return 扣款结果
	 */
	public Map<String, Object> cost(int money,String serialNO,String vmid,byte[] proid) {
		int timeOut = 15;//秒
...
//			System.arraycopy(seq, 0, seqBCD, 0, seqBCD.length);
			...
			byte[] time = Utils.decodeHex(sdf.format(new java.util.Date()));
			
			byte[] sendBytes = { (byte) 0x02, (byte) 0x26, (byte) 0xa2,
					...
					vmId2[8], vmId2[9],proid[0],proid[1],proid[2],proid[3],time[0],time[1],time[2],time[3],time[4],time[5],time[6],
					(byte)0x00, (byte)0x03 };
			sendBytes[sendBytes.length - 2] = getBCC(sendBytes);
			...
			while(is.available()<=0) {
				...
				if(endTime-startTime >= timeOut*1000){
					Logger.warn("receive data is timeout. "+timeOut+"s");
					return map;
				}
				Utils.sleep(100);
			}
...
//	}
	...
	public static String int2string2(int num,Integer len) {
		String tmp = "";
		for(int i =0;i<len;i++){
			tmp="0"+tmp;
		}
