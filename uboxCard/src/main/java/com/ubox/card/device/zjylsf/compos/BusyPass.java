...
import com.ubox.card.device.zjylsf.rs232.ZJYLSFSerial;
...
public class BusyPass {
    ...
    private static final ZJYLSFSerial rs232 = new ZJYLSFSerial(9600, RS232Worker.DATABITS_8, RS232Worker.STOPBITS_1, RS232Worker.PARITY_NONE);
    /**
     * 余额查询实现
     *
    ...
    public static String[] consumeOffline(int money) {
        ...
        if(contDomain[0][0] == ComClass.NAK) {
            ...
            Logger.info("Offline record: " + ZjfuUtil.BA2HS(record));
            ...
            int len = Integer.valueOf(ZjfuUtil.byte2HASCII(record[49]));
            ...
            Logger.info("Offline info: " +
                        "posId   = " + ZjfuUtil.BA2HS(posId)   + "," +
                        "mchntId = " + ZjfuUtil.BA2HS(mchntId) + "," +
                        "posSeq  = " + ZjfuUtil.BA2HS(posSeq)  + "," +
                        "account = " + ZjfuUtil.BA2HS(account) + "," +
                        "amount  = " + ZjfuUtil.BA2HS(amount)  + "," +
                        "time    = " + ZjfuUtil.BA2HS(time)    + "," +
                        "date    = " + ZjfuUtil.BA2HS(date)
            );
            ...
            ret[3] = ZjfuUtil.BA2HS(account).substring(0, len);
            ret[4] = String.valueOf(Integer.valueOf(ZjfuUtil.BA2HS(amount)));
            ret[5] = String.valueOf(Integer.valueOf(ZjfuUtil.BA2HS(posSeq)));
            ret[6] = ZjfuUtil.BA2HS(record);
            ret[7] = String.valueOf(Integer.valueOf(ZjfuUtil.BA2HS(date))) + String.valueOf(Integer.valueOf(ZjfuUtil.BA2HS(time)));

            return ret;
        } else {
            Logger.error(">>>>ERROR: Unknow CODE. CODE=" + ZjfuUtil.BA2HS(contDomain[0]) + "h");
        }
    ...
    public static String[] settle() {
        ...
        try {
            ...
            if(contDomain[0][0] == ComClass.NAK) {
                ...
                Logger.warn(">>>>ERROR: Unknow CODE. CODE=" + ZjfuUtil.BA2HS(contDomain[0]) + "h");
            }
        } catch (Exception e) {
