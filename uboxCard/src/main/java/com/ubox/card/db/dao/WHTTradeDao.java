...
public class WHTTradeDao extends BaseDao {

	private static final WHTTradeDao instance = new WHTTradeDao();
	
	private WHTTradeDao() {
	}
	
	public static WHTTradeDao getInstance() {
		return instance;
	}
	...
	public int insertOne(Object record) {
    	synchronized(this) {
    		if (!(record instanceof WHTTrade)) {// 参数为空或者参数不是需要的类型
    			return DB_FAIL;
    		}
    		...
    		String key = ((WHTTrade) record).getId();
    		records.put(key, (WHTTrade) record);
    	...
    	synchronized(this) {
    		...
    		if (records != null && records.size() != 0) {
    			...
    			for (String key : keySet) {
    				list.add(new WHTTrade(JSON.toJSONString(treeMap.get(key))));
    				i++;
    				if (i > max) {
