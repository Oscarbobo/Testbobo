...
public class QuickPassTradeDao extends BaseDao {

	private static final QuickPassTradeDao instance = new QuickPassTradeDao();
	
	private QuickPassTradeDao() {
	}
	
	public static QuickPassTradeDao getInstance() {
		return instance;
	}
	...
	public int insertOne(Object record) {
    	synchronized(this) {
    		if (!(record instanceof QuickPassTrade)) {// 参数为空或者参数不是需要的类型
    			return DB_FAIL;
    		}
    		...
    		String key = ((QuickPassTrade) record).getId();
    		records.put(key, (QuickPassTrade) record);
    	...
    	synchronized(this) {
    		...
    		if (records != null && records.size() != 0) {
    			...
    			for (String key : keySet) {
    				list.add(new QuickPassTrade(JSON.toJSONString(treeMap.get(key))));
    				i++;
    				if (i > max) {
